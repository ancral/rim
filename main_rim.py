...
train_gen = createAugment(x_train, x_train, mask=mask, batch_size=BATCH_SIZE,
                          num_sensors=NUM_SENSORS, type_scheme_mask=SHAPE_SENSORS, delete_data=DELETE_INDICES)
...
denoising_unet = Unet()
model = Rim(denoising_model=denoising_unet, padding_zone=padding_zone, rim_cfg=config["RIM"])
model.compile(
...
# Asegura que el modelo esté construido antes de guardar pesos
